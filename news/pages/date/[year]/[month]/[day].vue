                ...
                <div class="box">
                    <div class="box-header">
                        <h1 class="text-center">
                            Усі новини за <span class="title-marked">{{ route.params.day }}.{{ route.params.month }}.{{ route.params.year }} р.</span>
                        </h1>
                    </div>
...
const { articles, more, toPage, total, page, pages } = await getContentListContext({
    ...
    date: <string>`${route.params.year}-${route.params.month}-${route.params.day}`,
    per_page: 25,
});
...
const date = dayjs(`${route.params.year}-${route.params.month}-${route.params.day}`).locale('uk');
const month = ref(date.format('MMMM'));
const day = ref(date.format('D MMMM'));
...
const breadcrumbs = [
    ...
    {
        title: month.value,
        url: `/date/${route.params.year}/${route.params.month}`,
        className: 'capitalize',
    },
    {
        title: day.value,
        url: `/date/${route.params.year}/${route.params.month}/${route.params.day}`,
    },
];
