                ...
                <div class="box">
                    <div class="box-header">
                        <h1 class="text-center">
                            Усі новини за <span class="title-marked">{{ month }} {{ route.params.year }} р.</span>
                        </h1>
                    </div>
...
const { articles, more, toPage, total, page, pages } = await getContentListContext({
    ...
    date: <string>`${route.params.year}-${route.params.month}`,
    per_page: 25,
});
...
const month = ref(dayjs(`${route.params.year}-${route.params.month}-${1}`).locale('uk').format('MMMM'));
